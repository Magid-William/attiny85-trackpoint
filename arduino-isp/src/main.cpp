...
void loop(void) {
  if (pmode) { digitalWrite(LED_PMODE, HIGH); } else { digitalWrite(LED_PMODE, LOW); }
  if (ISPError) { digitalWrite(LED_ERR, HIGH); } else { digitalWrite(LED_ERR, LOW); }
  heartbeat();
  if (SERIAL.available()) {
    avrisp();
  }
...
void commit(unsigned int addr) {
  ...
  if (PROG_FLICKER) { delay(PTIME); prog_lamp(HIGH); }
}
