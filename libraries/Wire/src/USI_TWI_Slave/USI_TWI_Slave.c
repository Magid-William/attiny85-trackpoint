...
#endif
{
  ...
  DDR_USI &= ~(1 << PORT_USI_SDA); // Set SDA as input
  { // Exp43 patch: bound the SCL-low wait so a spurious start never blocks the CPU
    unsigned int w = 0;
    while ((tmpPin = (PIN_USI_CL & (1 << PORT_USI_SCL))) && ((PIN_USI & (1 << PIN_USI_SDA)) == 0))
      if (++w > 2000) { tmpPin = 0; break; } // treat as start, don't stall
  }
  if (tmpPin) {
  ...
  case USI_SLAVE_REQUEST_DATA:
    ...
    if (USI_TWI_On_Slave_Receive) {
      ...
      if (TWI_RxTail != tmpRxHead) { // data in receive buffer
        ...
        { // Exp43 patch: bound the start/stop wait so the ISR never blocks the CPU
          unsigned int w = 0;
          while ((USISR & ((1 << USI_START_COND_INT) | (1 << USIPF) | (0xE << USICNT0))) == 0)
            if (++w > 2000) break; // timeout: treat as no-stop, don't stall
        }
